...
/// <summary>
/// Multi-edition install integration test. Exercises the full pack -> install ->
/// filesystem-verify cycle against the example manifest in
/// <c>examples/exe-wrapper/multi-edition</c>.
/// </summary>
/// <remarks>
/// Tests report a genuine Skipped result (via <see cref="VmFactAttribute"/>, R6) when
...
public class MultiEditionInstallTests
{
    ...
    /// </summary>
    /// <remarks>
    /// Neither <c>/Edition=</c> nor <c>/InstallDir=</c> exists in the wrapper's closed
    /// grammar: a declared parameter is overridden with <c>/P&lt;name&gt;=&lt;value&gt;</c>
    /// and the install dir with <c>/D=</c> (R66). The example declares the parameter as
