...
/// <summary>
/// End-to-end prerequisite legs (G6), appended to the wrapper VM matrix. Packs a
/// self-contained fixture whose prerequisite <c>source</c> is a bundled copy of
/// <c>cmd.exe</c> (the "fake redist"): it writes an HKCU registry value (the detect
/// target — no file-path quoting) and exits with a chosen code. Covers:
/// <list type="bullet">
...
/// </list>
/// Reports a genuine Skipped result (via <see cref="VmPrerequisiteFactAttribute"/>, R6)
/// unless Windows + <c>SIGIL_VM_TESTS=1</c> + <c>SIGIL_VM_PREREQ=1</c> + the staged AOT
/// runtime. The runner decision logic is additionally covered by fast unit tests
/// (<c>PrerequisiteRunnerTests</c>).
/// </summary>
public sealed class PrerequisiteInstallTests
{
    ...
    /// <summary>
    /// A schema-valid app id for one run. The <c>r</c> prefix keeps the trailing hex
    /// segment letter-led, since <c>app.id</c>'s letter-led-segment pattern rejects a
    /// digit-led segment (R66).
    /// </summary>
        ...
        string id, string detectKey, int exitCode, string exitCodesOk)
    {
        ...
        // `detectKey` and `cmdArg` carry single backslashes: Sigil.YamlQuote emits
        // single-quoted YAML scalars, which have no escapes at all, so neither needs
        // hand-doubling (R66). $$ raw string: {{...}} interpolates, single braces
        // ({install_dir}) are literal.
        //
