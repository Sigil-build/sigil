...
patterns=(
  ...
  'Delete|^\s*Delete\b'
  'RmDir|^\s*RmDir\b'
  'WriteRegStr/DWORD/Bin/MultiStr/ExpandStr|^\s*Write(Reg(Str|DWORD|Bin|MultiStr|ExpandStr|None))\b'
  'DeleteRegValue|^\s*DeleteRegValue\b'
  'DeleteRegKey|^\s*DeleteRegKey\b'
  'CreateShortCut|^\s*CreateShortCut\b'
  'Exec/ExecWait/nsExec::Exec|^\s*(Exec|ExecWait|ExecShell)\b|nsExec::(Exec|ExecToLog|ExecToStack)\b'
