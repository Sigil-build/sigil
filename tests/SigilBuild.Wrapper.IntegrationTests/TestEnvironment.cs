...
/// the gate also keeps these out of the default developer test loop, where
/// launching the freshly-packed setup exe against the host filesystem would
/// require admin rights and the AOT-published wrapper runtime to be staged
/// under <c>runtimes/win-x64/</c>.
/// </summary>
internal static class TestEnvironment
{
    ...
    /// tests report a genuine <c>Skipped</c> result, never <c>Passed</c> (R6). The
    /// legacy <c>SigilBuild.Wrapper.exe</c> name predates the Avalonia host; it is
    /// accepted as a fallback for continuity.
    /// </summary>
